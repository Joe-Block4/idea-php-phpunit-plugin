 ...
 */
public class WillReturnIntention extends PsiElementBaseIntentionAction {
    @Override
    public void invoke(@NotNull Project project, Editor editor, @NotNull PsiElement psiElement) throws IncorrectOperationException {
        PsiElement parent = psiElement.getParent();
        if(parent instanceof MethodReference) {
            String name = ((MethodReference) parent).getName();

            PsiElement parent1 = ((MethodReference) parent).getFirstPsiChild();
            if(parent1 instanceof MethodReference && "method".equals(((MethodReference) parent1).getName())) {

                String parameter1 = CreateMockMethodReferenceProcessor.createParameter((MethodReference) parent1);


                PsiElement[] parameters = ((MethodReference) parent1).getParameters();
                if(parameters.length > 0 && parameters[0] instanceof StringLiteralExpression) {
                    String contents = ((StringLiteralExpression) parameters[0]).getContents();

                    String parameter = PhpUnitPluginUtil.findCreateMockParameterOnParameterScope((StringLiteralExpression) parameters[0]);
                    if(parameter != null) {
                        for (PhpClass phpClass : PhpIndex.getInstance(psiElement.getProject()).getAnyByFQN(parameter)) {
                            Method method = phpClass.findMethodByName(((StringLiteralExpression) parameters[0]).getContents());

                            for (String s : method.getType().getTypes()) {
        ...
        //PhpUnitPluginUtil.findCreateMockParameterOnParameterScope()
    ...
    public boolean isAvailable(@NotNull Project project, Editor editor, @NotNull PsiElement psiElement) {
        ...
        return parent instanceof MethodReference
            && "willReturn".equals(((MethodReference) parent).getName());
    }
    ...
    public String getText() {
        return "PHPUnit: Add return value";
    }
